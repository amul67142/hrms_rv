...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn('border-t border-border bg-surface-light font-medium [&>tr]:last:border-b-0', className)}
    {...props}
  />
))
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        'h-12 px-4 text-left align-middle font-medium text-white bg-surface-light [&:has([role=checkbox])]:pr-0',
        className
      )}
      style={{ color: '#FFFFFF' }}
      {...props}
